...
def findWaitingTime(processes, n, at, bt, rt, ft,
    ...
    while(1):
        ...
        for i in range(n):
            if (at[i] <= t):
                if (rem_bt[i] > 0) :
                    ...
                    if((bt[i] + (t-at[i]))/bt[i] < min_ratio):
                        min_i = i
                        min_ratio = (bt[i] + (t - at[i]))/bt[i]
        ...
        if(rt[min_i] == -1):
            ...
            wt[min_i] = t - bt[min_i] - at[min_i]
            ft[min_i] = at[min_i] + bt[min_i] + wt[min_i]
...
def findavgTime(processes, n, at, bt, quantum, st):
    ...
    while (minute <= 660):
        ...
        if(total_st > 0 and total_tat > 0):
            print("\nAverage service time = %.5f "%((total_st) / count) )
